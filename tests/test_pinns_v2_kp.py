"""
PINNs v2 Model: K-P Phase Diagram Test
UIと完全に同じ条件でテスト
"""
import sys
...
def test_pinns_v2_phase_diagram(
    nx=100,
    nt=200,
    L=10.0,
    T=10.0,
    c=1.0,
    save_path='tests/results/pinns_v2_phase_diagram.png'
):
    """Test PINNs v2 model and generate K-P phase diagram"""
    print("=" * 70)
    print("PINNs v2: K-P Phase Diagram Test")
    print("=" * 70)
    
    # Check model existence
    model_path = Path('models/checkpoints/pinns_v2.pth')
    if not model_path.exists():
        print(f"❌ Model not found: {model_path}")
        print("Please train the model first:")
        print("  python training/train_pinns_v2.py")
        return
    ...
    # Load model
    print(f"\nLoading model...")
    model = ModelFactory.create('pinns-v2')
    ...
    scatter = ax.scatter(K_history, P_history, c=time_points, 
                        cmap='coolwarm', s=50, alpha=0.7, edgecolors='black', linewidth=0.5)
    ...
    ax.set_title(f'PINNs v2: K-P Phase Diagram',
...
if __name__ == "__main__":
    results = test_pinns_v2_phase_diagram()
    ...
    if results:
        ...
        if results['E_variation'] < 5.0:
            print("Status: ✅ Excellent (Best in class!)")
